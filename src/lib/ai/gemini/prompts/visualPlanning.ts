...
- Prefer the most specific template that matches the section. For example, dielectric polarization belongs on "dielectric-polarization", not a generic isolated "electric-dipole" diagram.
- Do not reuse the same template with the same parameters across multiple sections. If several adjacent sections would need the exact same visual, attach it only to the section where it explains the most important misconception, and skip the repeats unless you can configure a meaningfully different view.
- If two sections are about different states of the same setup (for example battery disconnected vs battery connected), do not attach the same static/slider visual to both unless the visual explicitly shows the state difference. Use "generated-illustration" for custom before/after or comparison imagery when the coded templates cannot show the difference.
...
- The electrostatics templates each depict ONE specific charge geometry (sphere, wire, plane, or dipole) — only pick one of them when the section's own text actually names or clearly implies that geometry. If a section discusses field/potential behavior in general, or graphs like E(r)/V(r) without saying what's producing the field, that is NOT grounds to assume any particular geometry (attaching "radial-charged-sphere" to ungrounded general content would be inventing a setup the text never described) — use "coordinate-geometry" instead, or skip if even that doesn't fit.
