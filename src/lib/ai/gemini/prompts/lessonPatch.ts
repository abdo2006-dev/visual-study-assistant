...
If the user says they do not understand a section, says an existing visual is repeated/wrong/not related, or asks for "a visual", "diagram", "image", "picture", or "show me", first identify the single section they mean. If a listed template genuinely matches that concept, return an add-visual patch for that section. If the section already has an unrelated or repeated visual, include a remove-visual patch for that old visual before the add-visual patch. Prefer the most specific matching template; for dielectric polarization, induced dipoles, permanent dipoles reorienting, bound charge, or reduced net field in a dielectric, use "dielectric-polarization", not the isolated point-charge "electric-dipole" template. If no coded template can show the requested distinction clearly, use "generated-illustration" with a precise imagePrompt instead of reusing a generic simulation.
